    ...
    with ApiClient(config) as client:
        ...
        try:
            ...
            api.navigate_browser(
                browser_id=browser_id, payload={"url": "https://httpbin.org/forms/post", "timeout": 30}
            )

            # Wait for form to be ready
            print("Waiting for form...")
            api.wait_element(browser_id=browser_id, payload={"selector": "form", "until": "visible", "timeout": 10})

            # Fill text fields
            print("Filling form fields...")

            # Customer name
            api.type_text(browser_id=browser_id, payload={"selector": "input[name='custname']", "text": "John Doe"})

            # Telephone
            api.type_text(browser_id=browser_id, payload={"selector": "input[name='custtel']", "text": "555-1234"})

            # Email
            api.type_text(
                browser_id=browser_id, payload={"selector": "input[name='custemail']", "text": "john@example.com"}
            )

            # Click a radio button (pizza size)
            print("Selecting options...")
            api.click(browser_id=browser_id, payload={"selector": "input[value='medium']"})

            # Check a checkbox (topping)
            api.click(browser_id=browser_id, payload={"selector": "input[name='topping'][value='cheese']"})

            # Fill textarea
            api.type_text(
                browser_id=browser_id,
                payload={"selector": "textarea[name='comments']", "text": "Please deliver to the back door."},
            )
